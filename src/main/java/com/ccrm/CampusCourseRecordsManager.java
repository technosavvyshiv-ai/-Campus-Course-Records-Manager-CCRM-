...
public class CampusCourseRecordsManager {
    ...
    private static void generateStudentTranscript() {
        ...
        if (!includeInactive) {
            enrollments = enrollments.stream().filter(Enrollment::isActive).collect(Collectors.toList());
        }
        ...
        for (Enrollment e : enrollments) {
            Course c = dataStore.getCourse(e.getCourseId());
            if (c != null) {
                System.out.println(c.getCourseCode() + " - " + c.getTitle() + " (" + c.getCreditHours() + " credits)");
                System.out.println("  Grade: " + (e.getLetterGrade() != null ? e.getLetterGrade().getLetterGrade() : "N/A") +
                                   " (" + e.getNumericGrade() + ")");
                System.out.println("  Status: " + (e.isCompleted() ? "Completed" : "In Progress"));
    ...
    private static void addCourse() {
        ...
        Course course = new Course(courseId, courseCode, title, creditHours, instructorId, semester, department);
